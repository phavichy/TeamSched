...
from PyQt5.QtWidgets import QApplication, QWidget, QInputDialog, QLineEdit, QFileDialog, QMessageBox, QFileDialog, QPushButton, QListWidget
...
class WelcomePage(QWidget):
    ...
    def initUI(self):
        # Create a QListWidget to display the selected files
        ...
        # create a button to export df_final to .csv
        self.export_button = QPushButton("Export", self)
        self.export_button.clicked.connect(self.export_df_final)
        layout.addWidget(self.export_button)
        self.setLayout(layout)

    def get_pdf_files(self):
        self.pdf_files = QFileDialog.getOpenFileNames(self, "Select PDF files", "", "PDF Files (*.pdf)")[0]

        # Add the selected files to the QListWidget
        ...
        for col in df_all.columns:
            for i, row in df_all[col].items():
                ...
                if match:
                    df_passive = pd.concat([df_passive, pd.DataFrame(
                        {'Flight Number': [row], 'Date': [col], 'Pilot': df_all.loc[i, 'ID']})], ignore_index=True)
        ...
        for i, df_day in df_date.iterrows():
            ...
            df_csv.rename(
                index={'Flight Number': '', 'Pilot1': '', 'Pilot2': '', 'Pilot3': '', 'Pilot4': '', 'Pilot5': ''},
                inplace=True)
